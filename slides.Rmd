...
* Spatial structure of France for our simulation `r fontawesome::fa("arrow-right")` Admin1=region, admin2=department
...
* For each department we used the Brass relational model using as standard, the HMD country corresponding to the region to which it belongs, to generate its survival curve and hence, its mortality age schedule (__Show on map__)
...
* Simulated a and b over 10 years for both gender and used the Brass relational model to obtain survival curve and hence, mortality age schedule for a department. Repeat that process for each departments.
...
* Less volatility in generated than estimated parameters due to the scaling of covariance matrices
